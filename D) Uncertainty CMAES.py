...
print(f"\n===== SVR-CMAES UNCERTAINTY ANALYSIS  |  {config}  |  {csv_path} =====")
...
# ======================================================================
# Step 9: CMA-ES optimization  (fitness = VALIDATION MSE, test untouched)
# ======================================================================
print("\n9. CMA-ES OPTIMIZATION (fitness on validation subset)")

def svm_model_cmaes(params, X_train, X_val, y_train, y_val):
    C = 10 ** params[0]
    epsilon = 10 ** params[1]
    gamma = 'scale' if params[2] < 0 else 'auto'
    model = SVR(C=C, epsilon=epsilon, gamma=gamma, kernel='rbf')
    model.fit(X_train, y_train)
    return mean_squared_error(y_val, model.predict(X_val))

def cmaes_optimize(X_train, X_val, y_train, y_val, max_iterations=20):
    x0 = [np.log10(10), np.log10(0.1), 0]
    sigma = 0.5
    objective = lambda p: svm_model_cmaes(p, X_train, X_val, y_train, y_val)
    es = cma.CMAEvolutionStrategy(x0, sigma)
    es.optimize(objective, iterations=max_iterations)
    bp = es.result.xfavorite
    return 10 ** bp[0], 10 ** bp[1], ('scale' if bp[2] < 0 else 'auto')

best_C, best_epsilon, best_gamma = cmaes_optimize(X_train, X_val, y_train, y_val)
print(f"Best (on validation) - C: {best_C:.4f}, eps: {best_epsilon:.4f}, gamma: {best_gamma}")

def make_svr():
    return SVR(C=best_C, epsilon=best_epsilon, gamma=best_gamma, kernel='rbf')
...
# ======================================================================
print(f"\n12. FINAL POINT METRICS ({config}, SVR-CMAES)")
...
plt.title(f'SVR-CMAES {config}: test predictions with 95% prediction interval')
plt.legend(); plt.grid(True); plt.ylim(0, 8)
plt.savefig(f'{config}_predictions_PI_SVR-CMAES.png', dpi=200)
plt.close()
...
}).to_csv(f'{config}_uncertainty_results_SVR-CMAES.csv', index=False)

pd.DataFrame({
    'Configuration': [config], 'Model': ['SVR-CMAES'],
...
}).to_csv(f'{config}_metrics_SVR-CMAES.csv', index=False)

print(f"\nSaved results and metrics for {config} (SVR-CMAES).")
